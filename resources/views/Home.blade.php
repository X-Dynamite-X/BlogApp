    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            @foreach ($posts as $post)
                <article class="overflow-hidden rounded-xl bg-[#FDFDFC] shadow-lg transition-all duration-500 hover:scale-[1.02] hover:shadow-xl dark:bg-[#202020] dark:shadow-2xl cursor-pointer group flex flex-col h-[32rem]">
                    <a href="{{ route('post.show', $post) }}" class="flex-1 flex flex-col">
                        ...
                        {{-- Content Section - Flexible Height with Fixed Max Height --}}
                        <div class="p-6 flex-1 flex flex-col">
                            {{-- Title - Fixed Height --}}
                            <h3 class="mb-2 text-xl font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300 h-14 line-clamp-2">
                            ...
                            <p class="mb-4 text-gray-600 dark:text-gray-400 h-12 line-clamp-2">
                                {{ $post->content }}
                            </p>
                            ...
                            <div class="flex items-center h-12 mb-4">
                                ...
                                <div class="flex flex-col justify-center">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white line-clamp-1">{{ $post->user->name }}</p>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $post->created_at->diffForHumans() }}</p>
                                </div>
                            </div>

                            {{-- Interaction Buttons - Fixed Height --}}
                            <div class="flex items-center justify-between h-10 mt-auto">
                                <div class="flex items-center space-x-6">
                                    {{-- Like Button --}}
                                    <button class="like-btn group/btn flex items-center space-x-2"
                                        id="like-btn-{{ $post->id }}"
                                    ...
                                    {{-- Dislike Button --}}
                                    <button class="dislike-btn group/btn flex items-center space-x-2"
                                    ...
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span class="text-sm">{{ $post->views()->sum('view') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
